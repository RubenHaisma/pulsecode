...
export default function DashboardPage() {
  ...
  const fetchGitHubData = async (selectedTimeRange: string, forceRefresh: boolean = false) => {
    ...
    try {
      ...
      // Check if data comes from cache
      setIsFromCache(!!statsData.cached);
      // Update last refreshed timestamp if available
      if (statsData.stats && statsData.stats.lastRefreshed) {
        setLastRefreshed(new Date(statsData.stats.lastRefreshed));
      }
  ...
  // Add a LastRefreshed component to render in the dashboard
  const LastRefreshedInfo = () => {
    ...
    return (
      <div className="flex items-center text-sm text-muted-foreground">
        <RefreshCw className={`h-3 w-3 mr-1 ${isFromCache ? 'text-yellow-500' : 'text-green-500'}`} />
        <span>
          {isFromCache ? 'Cached data' : 'Fresh data'}, last updated {formatDate(lastRefreshed)}
        </span>
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background-start to-background-end">
      ...
      {/* Main content */}
      <div className="pl-64">
        <div className="container py-6 px-6 lg:px-8 mx-auto">
          {/* Render sign-in view if not signed in */}
          {!session && (
            <div className="p-8 flex flex-col items-center">
              <h1 className="text-2xl font-bold mb-8">Sign in to view your developer stats</h1>
              <Button onClick={() => signIn()}>Sign In</Button>
            </div>
          )}

          {/* Render initialization view */}
          {session && (!dataLastUpdated || fetchError) && (
            <div className="p-8 flex flex-col items-center max-w-lg mx-auto">
              <p className="text-xl mb-4">GitHub statistics</p>
              <div className="mb-8 text-center">
                {fetchError ? (
                  <div className="p-4 border border-red-500 rounded-lg text-red-500 mb-4">
                    {fetchError}
                  </div>
                ) : (
                  <p className="mb-4">Connect your GitHub account to fetch your coding stats.</p>
                )}
          ...
          {session && dataLastUpdated && !fetchError && (
            ...
            >
              {/* Header with refresh button and last refreshed info */}
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0 mb-6">
                <div>
                  <h1 className="text-2xl font-bold text-white pixel-font">Developer Dashboard</h1>
                  <LastRefreshedInfo />
                </div>
